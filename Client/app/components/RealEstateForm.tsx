...
export const RealEstateForm: React.FC<RealEstateFormProps> = ({ formData, onFormChange, onSubmit, isLoading }) => {
  ...
  const TOUR_STYLE_OPTIONS = ["Luxury Showcase", "Family-Friendly Walkthrough", "Modern & Sleek", "Cozy & Charming", "Investor's Brief"];

  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-1">
      <div>
        <h3 className="text-lg font-semibold text-sky-400 mb-3">Property Details (Fixed)</h3>
        <div className="space-y-2 p-3 bg-slate-700/50 rounded-lg border border-slate-600">
          <DetailItem label="Address" value={REAL_ESTATE_PROPERTY_DETAILS.address} />
          <DetailItem label="Price" value={REAL_ESTATE_PROPERTY_DETAILS.price} />
          <DetailItem label="Bedrooms" value={REAL_ESTATE_PROPERTY_DETAILS.bedrooms} />
          <DetailItem label="Bathrooms" value={REAL_ESTATE_PROPERTY_DETAILS.bathrooms} />
          <DetailItem label="Sq. Footage" value={`${REAL_ESTATE_PROPERTY_DETAILS.squareFootage.toLocaleString()} sq ft`} />
          <div className="pt-1">
             <span className="font-semibold text-slate-200">Features:</span>
             <p className="text-slate-300 text-sm mt-1">{REAL_ESTATE_PROPERTY_DETAILS.features}</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        >
          {TOUR_STYLE_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
        </select>
      </div>
      ...
      >
        {isLoading ? 'Generating Tour Script...' : 'Generate Real Estate Tour Script'}
      </button>
    </form>
  );
};
